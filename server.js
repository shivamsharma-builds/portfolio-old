...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    const allowed = /\.(jpg|jpeg|png|webp|gif|pdf|zip|doc|docx|txt)$/i.test(
      file.originalname,
    );
    cb(
      allowed ? null : new Error("Only image files and PDF files are allowed."),
      allowed,
    );
  },
});
...
async function ensureSchemaCompatibility() {
  ...
  for (const [c, d] of [
    ...
    ["offer_letter_url", "VARCHAR(1000) NULL"],
    ["offer_letter_file", "VARCHAR(1000) NULL"],
    ["sort_order", "INT DEFAULT 0"],
  ])
    await ensureColumn("experiences", c, d);
  for (const [c, d] of [
    ["title", "VARCHAR(255) NULL"],
    ...
    ["certificate_image", "VARCHAR(500) NULL"],
    ["sort_order", "INT DEFAULT 0"],
  ])
    await ensureColumn("certificates", c, d);
  await pool.query(
  ...
  for (const [c, d] of [
    ...
    await ensureColumn("education", c, d);
}
...
async function dynamicInsert(table, values, requiredAliases = {}) {
  ...
  for (const column of columns) {
    ...
    if (lower === "id" && String(column.EXTRA || "").includes("auto_increment"))
      continue;
    ...
    if (Object.prototype.hasOwnProperty.call(values, name)) {
      ...
      const alias = Object.keys(requiredAliases).find((a) =>
        requiredAliases[a].some((x) => x.toLowerCase() === lower),
      );
  ...
  if (!insertCols.length)
    throw new Error(`No insertable fields found in ${table}.`);
  const [result] = await pool.query(
...
function normalizedSite(row) {
  ...
  const headline = valueFor(
    map,
    ["headline", "title", "tagline", "role"],
    row,
    "",
  );
  return {
...
async function initDb() {
  ...
  if (!siteRows.length) {
    await dynamicInsert("site_content", {
      id: 1,
      name: "Shivam Sharma",
      headline: "Web Developer & C++ Programmer",
      intro:
      ...
      footer_description:
        "A personal portfolio showcasing my skills, projects, and passion for creating modern and meaningful digital experiences.",
    });
  }
  ...
  if (!skills.length) {
    const seed = [
      [
        "HTML Developer",
        "Crafting semantic, accessible websites with HTML5 for optimal performance.",
        "html.png",
        1,
      ],
      [
        "CSS Developer",
        "Creating responsive designs with CSS3, Flexbox, and Grid.",
        "css.png",
        2,
      ],
      [
        "JavaScript Developer",
        "Building dynamic web applications with ES6 and DOM manipulation.",
        "js.png",
        3,
      ],
      [
        "C++ Programmer",
        "Developing high-performance applications with OOP principles.",
        "c++.png",
        4,
      ],
      [
        "Python Developer",
        "Building scalable applications with Python and data analysis tools.",
        "python.png",
        5,
      ],
    ];
    for (const row of seed)
      await dynamicInsert("skills", {
        title: row[0],
        description: row[1],
        icon_file: row[2],
        icon_url: "",
  ...
  if (!projects.length) {
    const seed = [
      [
        "CLI Library Management System [CRUD]",
        "A command-line library management system using CRUD operations and MySQL to manage books, members, and transactions.",
        "c++.png",
      ...
      [
        ...
        "A JavaScript weather app that fetches live API data and displays temperature, humidity, conditions, and other weather details.",
        "js.png",
        "https://weather-main-xfqp.onrender.com",
        2,
      ],
      [
        "Crop Prediction Using AI",
        "An AI-powered agricultural project that predicts suitable crops using soil, weather, temperature, rainfall, and historical data.",
        "vite.png",
        "https://ai-crop-prediction.onrender.com",
        3,
      ],
      [
        "Spotify Clone",
        "A music-streaming UI inspired by Spotify, featuring song browsing, playlists, and playback-focused interface components.",
        "js.png",
        "https://spotifycloneshivam.netlify.app/",
        4,
      ],
    ];
    for (const row of seed)
      await dynamicInsert("projects", {
        title: row[0],
        description: row[1],
        icon_file: row[2],
        icon_url: "",
...
app.post("/api/logout", (req, res) =>
  req.session.destroy(() => res.json({ ok: true, redirect: "/admin.html" })),
);
...
app.get("/api/public", async (_req, res) => {
  try {
    const [[rawSite]] = await pool.query(
    ...
    const [certificates] = await pool.query(
      "SELECT * FROM certificates ORDER BY sort_order,id",
    );
    ...
    res.json({
      site: {
        ...site,
        profile_image: site.profile_image
          ? /^https?:\/\//i.test(site.profile_image)
            ? site.profile_image
            : fileUrl(site.profile_image)
          : "",
        resume_file: site.resume_file
          ? /^https?:\/\//i.test(site.resume_file)
            ? site.resume_file
            : fileUrl(site.resume_file)
          : "",
        hero_image: site.hero_image
          ? /^https?:\/\//i.test(site.hero_image)
            ? site.hero_image
            : fileUrl(site.hero_image)
          : "",
        about_image: site.about_image
          ? /^https?:\/\//i.test(site.about_image)
            ? site.about_image
            : fileUrl(site.about_image)
          : "",
      },
      skills: skills.map((s) => ({
        ...s,
        icon_file: s.icon_url || (s.icon_file ? fileUrl(s.icon_file) : ""),
      })),
      projects: projects.map((p) => ({
        ...
        icon_file:
          p.image_url ||
          p.icon_url ||
          (p.icon_file ? fileUrl(p.icon_file) : ""),
        project_file:
          p.project_url ||
          (p.project_file && /^https?:\/\//i.test(p.project_file)
            ? p.project_file
            : fileUrl(p.project_file)),
        github_url: p.github_url || p.project_url || "",
      })),
      experiences: experiences.map((e) => ({
        ...e,
        logo_image: e.logo_image
          ? isRemote(e.logo_image)
            ? e.logo_image
            : fileUrl(e.logo_image)
          : "",
        offer_letter_file: e.offer_letter_file
          ? isRemote(e.offer_letter_file)
            ? e.offer_letter_file
            : fileUrl(e.offer_letter_file)
          : "",
        offer_letter_url: e.offer_letter_url || "",
      })),
      certificates: certificates.map((c) => ({
        ...c,
        certificate_image:
          c.certificate_image && !/^https?:\/\//i.test(c.certificate_image)
            ? fileUrl(c.certificate_image)
            : c.certificate_image || "",
      })),
      education: education.map((e) => ({
        ...e,
        logo_image:
          e.logo_image && !/^https?:\/\//i.test(e.logo_image)
            ? fileUrl(e.logo_image)
            : e.logo_image || "",
      })),
    });
    ...
    try {
      ...
      if (!rows.length)
        return res
          ...
          .json({
            error:
              "Portfolio record is missing. Restart the server to initialize the database.",
          });
      ...
      const profile =
        ...
        null;
      const resume =
        req.files?.resume_file?.[0]?.filename ||
        current.resume_file ||
        fields.resume_url ||
        null;
      const hero =
        req.files?.hero_image?.[0]?.filename ||
        current.hero_image ||
        fields.hero_image_url ||
        null;
      const about = current.about_image || null;
      if (
        !String(fields.name || current.name || "").trim() ||
        !String(fields.headline || current.headline || "").trim()
      )
        return res
          .status(400)
          .json({ error: "Name and headline are required." });
      ...
      const values = {
        ...
        what_i_do: fields.what_i_do || "",
        goals: fields.goals || "",
        email: fields.email || "",
        github: fields.github || "",
        location: fields.location || "",
        profile_image:
          profile && !/^https?:\/\//i.test(profile)
            ? cleanFileName(profile)
            : null,
        resume_file:
          resume && !/^https?:\/\//i.test(resume)
            ? cleanFileName(resume)
            : null,
        profile_image_url:
          fields.profile_image_url ||
          (/^https?:\/\//i.test(profile || "") ? profile : ""),
        resume_url:
          fields.resume_url ||
          (/^https?:\/\//i.test(resume || "") ? resume : ""),
        hero_typed: fields.hero_typed || "",
        hero_image:
          hero && !/^https?:\/\//i.test(hero) ? cleanFileName(hero) : "",
        about_image:
          about && !/^https?:\/\//i.test(about) ? cleanFileName(about) : "",
        footer_description: fields.footer_description || "",
      };
      await dynamicUpdate("site_content", 1, values);
      res.json({ ok: true });
    } catch (e) {
      console.error("Admin site save failed:", e);
      res
        .status(500)
        .json({ error: e.message || "Could not save portfolio information." });
    }
  },
);
...
app.post(
  ...
  async (req, res) => {
    try {
      const { id, category, title, description, sort_order, icon_url } =
        req.body || {};
      ...
      if (id) {
        ...
        if (!rows.length)
          return res
            .status(404)
            .json({ error: "Skill not found. Reload the page and try again." });
        current = rows[0];
      }
      const uploaded = req.file?.filename || null;
      const iconFile =
        uploaded ||
        cleanFileName(req.body?.existing_icon_file) ||
        current.icon_file ||
        null;
      ...
      if (id) await dynamicUpdate("skills", id, values);
      else await dynamicInsert("skills", values);
      res.json({ ok: true });
    } catch (e) {
      console.error("Admin skill save failed:", e);
      res.status(500).json({ error: e.message || "Could not save skill." });
    }
  },
);
app.delete("/api/admin/skills/:id", auth, async (req, res) => {
  try {
    const [r] = await pool.query("DELETE FROM skills WHERE id=?", [
      req.params.id,
    ]);
    if (!r.affectedRows)
  ...
  async (req, res) => {
    try {
      ...
      if (id) {
        ...
        if (!rows.length)
          return res
            .status(404)
            .json({
              error: "Project not found. Reload the page and try again.",
            });
        current = rows[0];
      }
      ...
      const values = {
        ...
        sort_order: order,
        icon_file: iconFile,
        icon_url: icon_url || current.icon_url || "",
        image_url: image_url || current.image_url || "",
        project_url: project_url || current.project_url || "",
        github_url: github_url || current.github_url || "",
      };
      if (id) await dynamicUpdate("projects", id, values);
      else await dynamicInsert("projects", values);
      res.json({ ok: true });
    } catch (e) {
      console.error("Admin project save failed:", e);
      res.status(500).json({ error: e.message || "Could not save project." });
    }
  },
);
app.delete("/api/admin/projects/:id", auth, async (req, res) => {
  try {
    const [r] = await pool.query("DELETE FROM projects WHERE id=?", [
      req.params.id,
    ]);
    if (!r.affectedRows)
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!String(title || role || "").trim())
        return res.status(400).json({ error: "Experience title is required." });
      let current = {};
      if (id) {
        const [rows] = await pool.query(
      ...
      const logo =
        ...
        cleanFileName(req.body?.existing_logo_image) ||
        current.logo_image ||
        "";
      ...
      if (offerUpload && !/\.pdf$/i.test(offerUpload.originalname)) {
        ...
        return res
          .status(400)
          .json({ error: "Offer letter must be a PDF file." });
      }
      const offerFile =
        offerUpload?.filename ||
        cleanFileName(req.body?.existing_offer_letter_file) ||
        current.offer_letter_file ||
        "";
      const values = {
        role: String(title || role).trim(),
        title: String(title || role).trim(),
        company: company || "",
        logo_image: logo,
        description: description || "",
      ...
      if (id) await dynamicUpdate("experiences", id, values);
      else await dynamicInsert("experiences", values);
      res.json({ ok: true });
    } catch (e) {
      console.error("Admin experience save failed:", e);
      res.status(500).json({ error: e.message });
    }
  },
);
app.delete("/api/admin/experiences/:id", auth, async (req, res) => {
  try {
    const [rows] = await pool.query(
      "SELECT logo_image,offer_letter_file FROM experiences WHERE id=? LIMIT 1",
      [req.params.id],
    );
    const [r] = await pool.query("DELETE FROM experiences WHERE id=?", [
      req.params.id,
    ]);
    if (!r.affectedRows)
      return res.status(404).json({ error: "Experience not found." });
    for (const file of [rows[0]?.logo_image, rows[0]?.offer_letter_file]) {
      if (file)
        fs.rm(
          path.join(UPLOAD_DIR, path.basename(file)),
          { force: true },
          () => {},
        );
    }
  ...
  async (req, res) => {
    try {
      ...
      if (!String(title || "").trim())
        return res
          .status(400)
          .json({ error: "Certificate title is required." });
      let current = {};
      if (id) {
        const [rows] = await pool.query(
      ...
      const image =
        req.file?.filename ||
        cleanFileName(req.body?.existing_certificate_image) ||
        current.certificate_image ||
        "";
      const values = {
        title: String(title).trim(),
        issuer: issuer || "",
      ...
      if (id) await dynamicUpdate("certificates", id, values);
      else await dynamicInsert("certificates", values);
      res.json({ ok: true });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
...
app.delete("/api/admin/certificates/:id", auth, async (req, res) => {
  try {
    const [rows] = await pool.query(
      "SELECT certificate_image FROM certificates WHERE id=? LIMIT 1",
      [req.params.id],
    );
    const [r] = await pool.query("DELETE FROM certificates WHERE id=?", [
      req.params.id,
    ]);
    if (!r.affectedRows)
      return res.status(404).json({ error: "Certificate not found." });
    if (rows[0]?.certificate_image)
      fs.rm(
        path.join(UPLOAD_DIR, path.basename(rows[0].certificate_image)),
        { force: true },
        () => {},
      );
    res.json({ ok: true });
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      if (id) await dynamicUpdate("education", id, values);
      else await dynamicInsert("education", values);
      res.json({ ok: true });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
...
app.delete("/api/admin/education/:id", auth, async (req, res) => {
  try {
    const [rows] = await pool.query(
      "SELECT logo_image FROM education WHERE id=? LIMIT 1",
      [req.params.id],
    );
    const [r] = await pool.query("DELETE FROM education WHERE id=?", [
      req.params.id,
    ]);
    if (!r.affectedRows)
      return res.status(404).json({ error: "Education entry not found." });
    if (rows[0]?.logo_image)
      fs.rm(
        path.join(UPLOAD_DIR, path.basename(rows[0].logo_image)),
        { force: true },
        () => {},
      );
    res.json({ ok: true });
...
app.delete("/api/admin/site-image/:field", auth, async (req, res) => {
  try {
    const allowed = ["profile_image", "hero_image", "about_image"];
    ...
    const old = rows[0][req.params.field];
    await dynamicUpdate("site_content", 1, { [req.params.field]: null });
    if (old && !/^https?:\/\//i.test(old))
      fs.rm(
        path.join(UPLOAD_DIR, path.basename(old)),
        { force: true },
        () => {},
      );
    res.json({ ok: true });
