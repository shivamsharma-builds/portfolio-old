...
const cookieHeader = (value, maxAge = 0) =>
  `${SESSION_COOKIE}=${value}; Path=/; HttpOnly; SameSite=Lax; Secure; Max-Age=${maxAge}`;

function sign(value) {
  return crypto
    .createHmac("sha256", process.env.SESSION_SECRET || "change-this-secret")
    .update(value)
    .digest("base64url");
}
...
function localUrl(value) {
  return value
    ? isRemote(value)
      ? value
      : String(value).startsWith("/api/media/")
        ? value
        : `/uploads/${encodeURIComponent(String(value).replace(/^.*[\\/]/, ""))}`
    : "";
}
function storedUrl(value) {
  return value
    ? isRemote(value)
      ...
      : String(value).startsWith("/api/media/")
        ? value
        : `/uploads/${String(value).replace(/^.*[\\/]/, "")}`
    : "";
}

async function ensureSchema() {
  await db.query(
    `CREATE TABLE IF NOT EXISTS admins (id INT AUTO_INCREMENT PRIMARY KEY,email VARCHAR(255) NOT NULL UNIQUE,password_hash VARCHAR(255) NOT NULL,created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)`,
  );
  await db.query(
    `CREATE TABLE IF NOT EXISTS site_content (id INT PRIMARY KEY,name VARCHAR(255) NOT NULL,headline VARCHAR(255) NOT NULL,updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)`,
  );
  const siteCols = [
    ["intro", "LONGTEXT NULL"],
  ...
  const tables = {
    ...
    projects: [
      ...
      ["project_file", "VARCHAR(1000) NULL"],
    ...
    certificates: [
      ...
      ["certificate_image", "VARCHAR(1000) NULL"],
      ["sort_order", "INT DEFAULT 0"],
    ],
    education: [
  ...
  for (const [table, columns] of Object.entries({
    ...
    for (const column of columns) {
      await db.query(
        `ALTER TABLE \`${table}\` MODIFY COLUMN \`${column}\` VARCHAR(1000) NULL DEFAULT NULL`,
      );
    }
  }
  ...
  if (!site)
    await db.query(
      "INSERT INTO site_content (id,name,headline) VALUES (1,?,?)",
      ["Shivam Sharma", "Web Developer & C++ Programmer"],
    );
}
let schemaPromise;
function readySchema() {
  return (schemaPromise ||= ensureSchema());
}
...
async function uploadFile(file, folder = "media") {
  ...
  if (!allowed.has(file.type))
    throw new Error(
      "Unsupported upload type. Use JPG, PNG, WEBP, GIF, or PDF.",
    );
...
function fieldMap(form) {
  const out = {};
  for (const [k, v] of form.entries()) if (!(v instanceof File)) out[k] = v;
  return out;
}
...
async function publicData() {
  ...
  const [[skills], [projects], [experiences], [certificates], [education]] =
    await Promise.all([
      db.query("SELECT * FROM skills ORDER BY sort_order,id"),
      db.query("SELECT * FROM projects ORDER BY sort_order,id"),
      db.query("SELECT * FROM experiences ORDER BY sort_order,id"),
      db.query("SELECT * FROM certificates ORDER BY sort_order,id"),
      db.query("SELECT * FROM education ORDER BY sort_order,id"),
    ]);
  return {
    site: {
      ...site,
      profile_image: site.profile_image_url || storedUrl(site.profile_image),
      resume_file: site.resume_url || storedUrl(site.resume_file),
      hero_image: storedUrl(site.hero_image),
      about_image: storedUrl(site.about_image),
    },
    skills: skills.map((x) => ({
      ...x,
      icon_file: x.icon_url || storedUrl(x.icon_file),
    })),
    projects: projects.map((x) => ({
      ...x,
      icon_file: x.image_url || x.icon_url || storedUrl(x.icon_file),
      project_file: x.project_url || storedUrl(x.project_file),
      github_url: x.github_url || x.project_url || "",
    })),
    experiences: experiences.map((x) => ({
      ...x,
      logo_image: storedUrl(x.logo_image),
      offer_letter_file: storedUrl(x.offer_letter_file),
      offer_letter_url: x.offer_letter_url || "",
    })),
    certificates: certificates.map((x) => ({
      ...x,
      certificate_image: storedUrl(x.certificate_image),
    })),
...
async function saveRow(table, id, values) {
  const allowed = new Set(
    (await db.query(`SHOW COLUMNS FROM \`${table}\``))[0].map((x) => x.Field),
  );
  const entries = Object.entries(values).filter(([k]) => allowed.has(k));
  if (!entries.length)
    throw new Error(`No compatible fields found in ${table}.`);
  if (id) {
    await db.query(
...
async function handleSite(request) {
  ...
  if (
    !String(f.name || current.name || "").trim() ||
    !String(f.headline || current.headline || "").trim()
  )
    throw Object.assign(new Error("Name and headline are required."), {
      status: 400,
    });
  const values = {
    name: String(f.name || current.name).trim(),
    headline: String(f.headline || current.headline).trim(),
    intro: f.intro || "",
    about_title: f.about_title || "",
    about_text: f.about_text || "",
    what_i_do: f.what_i_do || "",
    goals: f.goals || "",
    email: f.email || "",
    github: f.github || "",
    location: f.location || "",
    hero_typed: f.hero_typed || "",
    footer_description: f.footer_description || "",
  };
  for (const [field, urlField, folder] of [
    ["profile_image", "profile_image_url", "profile"],
    ["hero_image", "hero_image_url", "hero"],
  ]) {
    const file = form.get(field);
    const uploaded = await uploadFile(file, folder);
    const remote = f[urlField] || "";
    if (uploaded) {
      await deleteMedia(current[field]);
      values[field] = uploaded;
      values[urlField] = "";
    } else if (remote) {
      await deleteMedia(current[field]);
      values[field] = isRemote(remote) ? remote : null;
      values[urlField] = isRemote(remote) ? remote : "";
    } else values[field] = current[field] || null;
  }
  const resume = form.get("resume_file");
  const resumeUpload = await uploadFile(resume, "resume");
  const remoteResume = f.resume_url || "";
  if (resumeUpload) {
    await deleteMedia(current.resume_file);
    values.resume_file = resumeUpload;
    values.resume_url = "";
  } else if (remoteResume) {
    await deleteMedia(current.resume_file);
    values.resume_file = isRemote(remoteResume) ? remoteResume : null;
    values.resume_url = isRemote(remoteResume) ? remoteResume : "";
  } else values.resume_file = current.resume_file || null;
  await saveRow("site_content", 1, values);
  return json({ ok: true });
}
...
async function handleExperience(request) {
  ...
  if (!String(f.title || f.role || "").trim())
    throw Object.assign(new Error("Experience title is required."), {
      status: 400,
    });
  let current = {};
  if (id) {
    ...
    if (!row)
      throw Object.assign(new Error("Experience entry not found."), {
        status: 404,
      });
    current = row;
  }
  ...
  if (logo) {
    await deleteMedia(f.existing_logo_image || current.logo_image || "");
    f.logo_image = logo;
  ...
  if (offer instanceof File && offer.size) {
    if (offer.type !== "application/pdf" && !/\.pdf$/i.test(offer.name))
      throw Object.assign(new Error("Offer letter must be a PDF file."), {
        status: 400,
      });
    const uploaded = await uploadFile(offer, "experiences/offer-letters");
    await deleteMedia(
      f.existing_offer_letter_file || current.offer_letter_file || "",
    );
  ...
  f.offer_letter_url = "";
  delete f.id;
  delete f.existing_logo_image;
  delete f.existing_offer_letter_file;
  delete f.logo_image_url;
...
async function handleEntity(
  ...
  const f = fieldMap(form);
  const id = f.id ? Number(f.id) : null;
  if (!String(f[requiredField] || "").trim())
    throw Object.assign(
      new Error(
        `${requiredField === "institution" ? "Institution" : requiredField[0].toUpperCase() + requiredField.slice(1)} is required.`,
      ),
      { status: 400 },
    );
  let current = {};
  if (id) {
    ...
    if (!row)
      throw Object.assign(new Error(`${table} entry not found.`), {
        status: 404,
      });
    current = row;
  }
  ...
  const uploaded = await uploadFile(file, folder);
  const old = f[`existing_${fileField}`] || current[fileField] || "";
  if (uploaded) {
    await deleteMedia(old);
    f[fileField] = uploaded;
  } else f[fileField] = old;
  Object.assign(f, extra);
  if (table === "skills" && f.icon_url === undefined) f.icon_url = "";
  if (table === "experiences") f.role = String(f.title || f.role || "").trim();
  delete f.id;
  delete f[`existing_${fileField}`];
  await saveRow(table, id, f);
  return json({ ok: true });
}
...
async function handleProject(request) {
  ...
  if (!String(f.title || "").trim())
    throw Object.assign(new Error("Project title is required."), {
      status: 400,
    });
  ...
  if (id) {
    ...
    if (!row)
      throw Object.assign(new Error("Project entry not found."), {
        status: 404,
      });
    current = row;
  }
...
async function main(request) {
  ...
  path =
    path.replace(/^\/\.netlify\/functions\/api/, "").replace(/^\/api/, "") ||
    "/";
  ...
  if (path === "/media" || path.startsWith("/media/")) {
    ...
    const metadata = await uploads.getMetadata(key);
    if (!metadata) return text("Not found", 404);
    const body = await uploads.get(key, { type: "arrayBuffer" });
  ...
  if (path === "/session" && method === "GET")
    return json({
      authenticated: !!sessionEmail(request),
      email: sessionEmail(request),
    });
  if (path === "/login" && method === "POST") {
    ...
    return json({ ok: true, redirect: "/admin.html" }, 200, {
      "set-cookie": cookieHeader(
        createSession(rows[0].email),
        SESSION_TTL / 1000,
      ),
    });
  }
  ...
  if (path === "/public" && method === "GET")
    return json(await publicData(), 200, {
  ...
  if (path === "/admin/site-image" && method === "DELETE")
    return json({ error: "Use /api/admin/site-image/:field" }, 400);
  const siteDelete = path.match(/^\/admin\/site-image\/([^/]+)$/);
  if (siteDelete && method === "DELETE") {
    requireAuth(request);
    const field = siteDelete[1];
    if (!["profile_image", "hero_image", "about_image"].includes(field))
      return json({ error: "Invalid image field." }, 400);
    const site = await getSite();
    await deleteMedia(site[field]);
    await saveRow("site_content", 1, { [field]: null, [`${field}_url`]: "" });
    return json({ ok: true });
  }
  const entity = path.match(
    /^\/admin\/(skills|projects|experiences|certificates|education)(?:\/(\d+))?$/,
  );
  if (entity) {
    const table = entity[1],
      id = entity[2] ? Number(entity[2]) : null;
    if (method === "DELETE") {
      requireAuth(request);
      const [[row]] = await db.query(
        `SELECT * FROM \`${table}\` WHERE id=? LIMIT 1`,
      ...
      for (const field of [
        ...
        if (row[field]) await deleteMedia(row[field]);
      await db.query(`DELETE FROM \`${table}\` WHERE id=?`, [id]);
      return json({ ok: true });
    }
    if (method === "POST") {
      if (table === "skills")
        return handleEntity(request, table, "icon_file", "skills", "title");
